package it.univaq.disim.topfilter.validation;
...
import it.univaq.disim.topfilter.DataReader;

public class Metrics {
	final static Logger logger = Logger.getLogger(Metrics.class);

	private static final int _NUM_OF_MNBN_TOPIC = 5;
...
//				writer2.close();
	...
	public void successRateB(int numberOfTopicsFromEASE) {
		...
		for (Integer keyTesting : keyTestingProjects) {
			...
			// JURI
			String trainingDictFilename = Paths.get(this.srcDir, "dicth_" + filename).toString();
			groundTruthData = reader.getLibraries(trainingDictFilename);
	...
	public void computeAverageSuccessRateB() {
		...
		int size = testingProjects.size();
		String successRateAgrregatedResult = Paths.get(this.resDir, "SRB" + "_Round" + Integer.toString(fold))
				.toString();
	...
	public void computeAverageSuccessRate() {
		...
		for (Integer keyTesting : keyTestingProjects) {
			...
			try (BufferedReader bufread = new BufferedReader(new FileReader(tmp))) {
				...
				if (id < numLibs)
					for (int i = id; i <= numLibs; i++) {
						if (successRateMap.containsKey(id))
							val2 = successRateMap.get(id) + successRate;
						successRateMap.put(i, val2);
					}
			} catch (IOException e) {
				e.printStackTrace();
			}
...
//					logger.info(val);
	...
	public void catalogCoverageB() {
		Set<Integer> keyTestingProjects = testingProjects.keySet();

		int allItems = getAllItems().size();
		Map<Integer, Set<String>> enneSizeMap = Maps.newHashMap();
		for (int i = 1; i <= 10; i++) {
			for (Integer keyTesting : keyTestingProjects) {

				String testingPro = testingProjects.get(keyTesting);
				String testingRecsPath = Paths.get(this.recDir, testingPro).toString();
				// String testingProjectPath = Paths.get(this.srcDir, testingPro).toString();
				List<String> easeT = reader.getEASETopic(testingPro, _NUM_OF_MNBN_TOPIC);
				List<String> recs = Lists.newArrayList(reader.readRecommendationFile(testingRecsPath, i));
				easeT.addAll(recs);
				try {
					if (easeT.size() >= i)
						if (enneSizeMap.get(i) == null)
							enneSizeMap.put(i, Sets.newHashSet(easeT.subList(0, i)));
						else
							enneSizeMap.get(i).addAll(easeT.subList(0, i));
					else if (enneSizeMap.get(i) == null)
						enneSizeMap.put(i, Sets.newHashSet(easeT));
					else
						enneSizeMap.get(i).addAll(easeT);
				} catch (Exception e) {
					logger.error(i + " " + testingPro.replace("___", "/") + " " + e.getMessage());
					throw e;
				}

			}
		}
		String tmp = Paths.get(this.resDir, "Catalog" + fold).toString();
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(tmp))) {
			for (Map.Entry<Integer, Set<String>> i : enneSizeMap.entrySet()) {
				writer.write(i.getKey() + "\t" + (i.getValue().size() * 1.0) / allItems);
				writer.newLine();
			}
		} catch (IOException e) {
		...
		for (int i = 1; i <= 10; i++) {
			for (Integer keyTesting : keyTestingProjects) {
				...
				try {
					if (recs.size() >= i)
						if (enneSizeMap.get(i) == null)
							enneSizeMap.put(i, Sets.newHashSet(recs));
						else
							enneSizeMap.get(i).addAll(recs.subList(0, i));
					else if (enneSizeMap.get(i) == null)
						enneSizeMap.put(i, Sets.newHashSet(recs));
					else
						enneSizeMap.get(i).addAll(recs);
				} catch (Exception e) {
					logger.error(i + " " + testingPro.replace("___", "/") + " " + e.getMessage());
					throw e;
				}
