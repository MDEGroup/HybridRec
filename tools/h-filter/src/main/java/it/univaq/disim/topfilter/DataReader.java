...
//			half = size > 1 ? 1 : size;
	...
	public List<String> getEASETopic(String projectName, int n) {
		if (eASEOutput.size()==0)
			loadEASEOutput();
		return eASEOutput.get(projectName.replace("___", "/")).stream().limit(n).collect(Collectors.toList());
	}
	public void loadEASEOutput() {
		String filename = Paths.get(this.srcDir, "training_data.csv").toString();
		try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
			String line;
			while ((line = reader.readLine()) != null) {
	...
	}//artifact/commons-httpclient.wso2/commons-httpclient
